...
/**
 * Xiaohongshu connector — extension-type.
...
export class XiaohongshuConnector implements PlatformConnector {
  private readonly logger = new Logger(XiaohongshuConnector.name);
   ...
   * A connection is healthy if last_sync_at is within HEARTBEAT_STALE_MS.
   */
   ...
   * Expects rawData to be an array of items matching the extension-sync.md format.
   */
  parseResponse(rawData: unknown): ContentItemInput[] {
    ...
    for (const raw of rawData) {
      ...
      items.push({
        ...
        content_type: (item.content_type as string) || 'post',
